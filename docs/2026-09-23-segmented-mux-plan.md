...
> **状态（2026-09-23）**：**机制已实现并验收通过**（提交 `2bdfb46`）——
> `ALH_TEST_SEGMENT_FRAMES=<n>` 强制分段；A/B 实测元数据完全一致（2560×1440 · 60fps · 3.000000s · 180 帧）、
> 逐帧 PSNR 最低 **52.46 dB** / 平均 68.5 dB（肉眼无差别）、**错开一帧比对掉到 27.54 dB** ⇒ 接缝无丢帧无重帧。
> 自检还抓出并修掉了"统计去数目录 ⇒ 分段删帧后报 `实际输出:0 帧`"这个真问题。
> **仍未做**：① **边处理边编码**（把峰值从"全片最终帧"压到"段"的那一步；当前是"处理完再分段编码"，
> 所以峰值还没降）② 在飞段数背压（c+1）③ 接进守门自动触发（Task 3）。
