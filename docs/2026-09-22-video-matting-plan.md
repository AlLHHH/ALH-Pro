...
## 执行记录（2026-09-22 当天，Task 1–6 全部落地）

| 任务 | 状态 | 提交 | 偏离计划的地方（都是实测/读码发现的，不是口味问题） |
|---|---|---|---|
| Task 1 性能标定 | ✅ | 报告在 `_qa/视频抠图_性能实测_20260922.md`（`_qa` 被 gitignore，结论已回写设计文档 §2.4） | ① 全量扫描第一遍被"DML 污染"带偏（同进程先跑 birefnet-lite 后，后面几支模型的 GPU 数字全失真）⇒ **测 DML 必须一个模型一个进程**；② 实测推翻计划前提"必须走 GPU"：isnet/u2net 的 GPU 确实快 2.4~2.6 倍，但 birefnet-lite 的 DML 路是坏的（且失败后同进程全部退化成 CPU 速度） |
| Task 2 输出规格表 | ✅ | `7cdd826` + `72c8cfa` | 换背景一路**不写死编码器**（交现有 `EncoderArgs` 策略，避免双策略漂移）；音频字段改参数片段并区分 copy/Opus/PCM；测试 5 → 22 条。透明通道**默认档定为 MOV/ProRes 4444**（实测 WebM 的 alpha 连 ffmpeg 原生 vp9 解码器都会静默丢） |
| Task 3 alpha 后处理 | ✅ | `f311086` | 计划里的**羽化判据数学上恒不成立**（线性斜坡是方框模糊的不动点，模糊前后相邻差都是 0.125）⇒ 改用硬台阶输入；形态学改**可分离**（O(n·r²)→O(n·r)）并补 property 测试证明与朴素二维实现逐像素等价；补坏输入/值域测试。测试 3 → 7 条 |
| Task 4 时序稳定 | ✅ | `dd32a7b` | "静止更稳"那条用计划给的常数（gate 4f）未达标 ⇒ 按计划授权把 gate 系数下调到 3.5f（判据未动）；补 5 条判据：实心区立刻跟随（直接对着"拖影"测）、抖动收敛到中值、短帧不污染历史、滑条单调性、空/异长输入不抛。测试 5 → 10 条 |
| Task 5 合成 | ✅ | `9ee0139` | 计划少了 **dst 长度检查**（调用方复用缓冲区长度不匹配 = 越界写内存）⇒ 补上并测试"一个字节都不写"；另补多像素步进、alpha 越界夹取、四舍五入。测试 4 → 8 条 |
| Task 6 收尾 | ✅ | 见上 | 全量回归 **1131 项 / 1130 通过**（唯一失败是既有的 `NavLogoWebsiteTests`）；实测结论已回写设计文档 §2.4/§四/§八 |

**执行期新增/修正的设计条目**（已写进 `docs/2026-09-22-video-matting.md`）：视频路径需要 `CutoutService` 新增
"只出蒙版"入口（不重复后处理、不每帧写 PNG）；ETA 系数口径修正（§一 的 0.40 s/帧是整条 `CutoutAsync`，
不是纯推理 —— 新口径"待定 ≥ 纯推理 + 0.096 s/帧"，**开工前补测**）。

**本阶段总产出**：`AlhPro.Core/VideoMatting.cs`（alpha 后处理 + 时域滤波 + 合成）、
`AlhPro.Core/MattingOutputSpec.cs`、`AlhPro.Core/DmlAttemptLedger.cs`，加 5 个测试文件共 **57 条新测试用例**
（测试总数 1074 → 1131，差值正好对上）；顺带修掉图片抠图页的 DirectML 静默降级缺陷并把默认模型换成 `isnet-general-use`。

**下一阶段（独立计划，尚未编写）**：`VideoMattingService`（帧循环/会话复用/临时盘/ffmpeg 对接）、
**独立的「视频抠图」页面**（`Views/VideoMattingView.xaml` + 导航第 5 项）、`CutoutService` 的"只出蒙版"入口、
以及文档/发版同步。
