        ...
        if (forceGrid)
        {
            ...
            try
            {
                ...
                if (pairN >= 3)
                {
                    ...
                    if (meanSad > 3.5)
                    {
                        progress?.Report((4, $"⚠ 档位校验:相邻内容帧平均差异 {meanSad:0.#}(较大)——该素材实际内容帧率明显高于填写值(实拍被抽稀,补帧只能靠猜),建议改用「智能检测」;当前仍按本次判定处理"));
                        AppLogger.Info($"档位校验:相邻内容帧平均差异 {meanSad:0.#}(较大) → 填写的内容帧率可能偏高(实拍被抽稀),建议改用「智能检测」");
                    }
                }
            }
            catch { /* 校验失败忽略 */ }
