...
public sealed partial class MainPage : Page
{
    ...
    public MainPage()
    {
        ...
        Loaded += async (_, _) =>
        {
            ...
            // 自检弹窗:仅首次启动 / 版本更新后显示(用刚点过确定的标记,避免 RunOnce 竞态导致首次被跳过)
            bool needFullCheck = !AppSettings.SelfCheckDone
    ...
    private void SelfCheckOk_Click(object sender, Microsoft.UI.Xaml.RoutedEventArgs e)
    {
        ...
        AppSettings.SelfCheckDone = true;   // 自检已完成(点确定),后续非首次/更新不再弹
        try { AppSettings.Save(); } catch { }
    ...
    /// <summary>兜底占位图 BitmapImage;文件缺失也返回 null(调用方再兜底隐藏)。</summary>
    private static BitmapImage? AdPlaceholderImage()
    {
        try
        {
            if (System.IO.File.Exists(AdPlaceholderPath))
                return new BitmapImage(new Uri(AdPlaceholderPath));
        }
        catch { }
        return null;
    }
    ...
    private void About_Click(object sender, RoutedEventArgs e)
    {
        ...
        // 【2026-09-13】个人致谢不写在这里 —— 关于页的「特别致谢(不分先后)」名单是【自动扫描程序目录 thanks\ 文件夹】
        // 生成的:文件名 = 显示的网名,`.txt` = 只写名字、`.png/.jpg` = 带头像。新增一位只需在 thanks\ 放一个文件。
        // 「记住羡鱼」就是这么加的:thanks\记住羡鱼.txt(仓库两处:ImgUpscalerUI\thanks\ 与 发布版\thanks\)。
        // 【踩过的坑】不要像本文件早先那样直接 content.Children.Add 一行文本:会和既有名单重复显示,
        // 而且以后加头像/排序都没地方挂 —— 走 thanks\ 才是这套名单的设计口径。
        // 许可声明可直接点开
