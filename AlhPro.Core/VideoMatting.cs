...
/// <summary>逐帧蒙版(alpha)处理的纯逻辑件:后处理 / 时域滤波 / 合成。
///
/// 【2026-09-23:视频抠图功能已整条下线,但这份纯逻辑**有意保留**】
/// 用户定案「视频抠图功能删除,留一点你认为有用的、有助于发展的;不过视频抠图以后不会再做了」。
/// 保留的理由:这三段算法**不依赖 UI / 显卡 / ffmpeg**,只吃 0~1 的浮点蒙版 ⇒ 将来做
/// ①图片抠图的边缘质量(同一套"阈值 / 羽化 / 形态学"数学)、②任何逐帧蒙版的稳定化,都能直接用:
/// 实测(1080p)后处理默认档 82 ms/帧、时域滤波只 3.9 ms/帧、合成 10.3 ms/帧。
/// 【当前状态必须说清楚】它的**唯一消费者就是下面这三个测试文件**(Alpha/Composite/TemporalFilter),
/// 生产路径已经没有调用方 —— 这是"刻意留存的工具件",不是"忘了删的死代码"。
/// 来龙去脉与全部实测数字见 `docs/2026-09-23-视频抠图下线-保留的经验与可复用件.md`。
/// 【口径】参数仍是图片抠图页那一套 0~255 整数滑条与同一批档位,免得两处对同一份蒙版给出不同语义。
/// </summary>
...
///
/// 三个机制(2026-09-23 原设计文档随功能下线删除,摘要见
/// `docs/2026-09-23-视频抠图下线-保留的经验与可复用件.md` §二.1):
