...
/// ==== 安全边际 ====
/// 预估节省 **&lt; 15%** 时**不切换**(避免在临界点上抖动/来回翻),理由写进日志。</summary>
public static class PipelineOrderPlan
{
    ...
    /// <summary>安全边际:预估节省低于它就不切换顺序(15%)。</summary>
    public const double MinSavingsPercent = 15.0;

    /// <summary>超分单价表的一行(秒/帧 @1080p 源)。</summary>
    public readonly record struct UpscaleRate(string ModelKey, int EngineScale, double SecondsPerFrame1080p, string Provenance);

    /// <summary>实测超分单价表(出处逐条标注;区间取中值,区间宽度见 Provenance)。</summary>
    public static readonly UpscaleRate[] UpscaleRates =
    {
        new("animevideov3", 1, 0.028,  "2026-09-13 实测 0.027~0.029;【该档输出全黑(无 x1 权重),数字仅供量级】"),
        new("animevideov3", 2, 0.2605, "2026-09-13 实测 0.252~0.269"),
        new("animevideov3", 4, 0.297,  "2026-09-13 实测 0.294~0.300"),
        new("general-x4v3", 4, 0.458,  "2026-09-13 实测 0.455~0.461"),
        new("x4plus", 4, 15.145,       "2026-09-13 实测 14.82~15.47;【仅 12 帧样本,不确定性最大】"),
        new("x4plus-anime", 4, 3.85,   "2026-09-13 实测 ≈3.3~4.4(区间较宽,取中值)"),
        new("cunet", 2, 0.3685,        "2026-09-13 实测 -n0 0.368~0.370 / -n1 0.373~0.383 / -n2 0.353~0.364(三档中值)"),
        new("upconv_7_photo", 2, 1.288, "2026-09-13 实测 ≈1.288"),
    };

    /// <summary>把模型名归一到成本表的键(与表里的 ModelKey 对应);认不出返回 null。</summary>
    public static string? NormalizeModel(string? model)
    {
        string m = model ?? "";
        if (m.Contains("x4plus-anime", StringComparison.OrdinalIgnoreCase)) return "x4plus-anime";   // 必须先于 x4plus 判
        ...
        if (m.Contains("general-x4v3", StringComparison.OrdinalIgnoreCase)) return "general-x4v3";
    ...
    /// <summary>查"该模型 × 该引擎倍率"的实测超分单价(秒/帧 @1080p 源);没实测过返回 null(调用方回退旧顺序)。</summary>
    ...
    /// <summary>补帧"每个输出帧"的成本(秒):按面积在实测锚点之间分段线性内插,区间外按相邻段斜率外推(带极小值兜底)。</summary>
    public static double InterpSecondsPerOutputFrame(long pixels)
    {
        if (pixels <= 0) return InterpAnchorSeconds[0];
        var px = InterpAnchorPixels;
        var sc = InterpAnchorSeconds;
    ...
    public static Decision Decide(string? engine, string? model, double scale, int interpScale, int srcW, int srcH, int sourceFrames = 900,
        double areaScale = 0, double minSavingsPercent = MinSavingsPercent)
    {
    ...
    /// ③按判据算两侧总成本并比较;④新顺序更省但节省 &lt;<see cref="MinSavingsPercent"/>% → 仍保持旧顺序(边际不足)。</summary>
    public static Decision Decide(CostInput cost, double scale, int interpScale, int srcW, int srcH, int sourceFrames = 900,
        double minSavingsPercent = MinSavingsPercent, double areaScale = 0)
    {
