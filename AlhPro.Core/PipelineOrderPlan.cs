...
///     · 2x = **0.252~0.269** · 3x = **0.264~0.299** · 4x = **0.294~0.300**
///   · realesr-general-x4v3  4x = **0.455~0.461**
///   · realesr-x4plus        4x = **15.87**【2026-09-15 起为 **30 帧目录批跑**的大样本;同机 4 次样本 10.19~19.05,差异全部来自笔记本 GPU 热降频】
...
///
/// ==== 【2026-09-15 面积线性验证:同一模型、同一内容、同样 12 帧,1080p 源 vs 2160p 源】====
///   · animevideov3 2x:1080p = **0.296~0.303** s/帧、2160p = **1.006~1.016** s/帧 → 原始比值 **3.36~3.40**
///   · general-x4v3  4x:1080p = **0.522~0.550** s/帧、2160p = **1.832~1.852** s/帧 → 原始比值 **3.37~3.51**
...
///   1080p:k=2 → **0.059~0.071**、k=4 → **0.076~0.088** 秒/输出帧;
///   2160p:k=2 → **0.234~0.237**、k=4 → **0.337~0.338** 秒/输出帧。⇒ **1080p/2160p 两个锚点都落在实测区间内,
///   锚点值可以继续用**;但 1080p→2160p 这段的面积比实测是 **3.40~4.43**(面积涨 4 倍)—— 是"近似正比"而不是
///   "明显次线性",所以"次线性"这句只对 2160p→4320p 那一档成立(见下)。
///   **4320p 只有一个样本且偏贵**:k=2 实测 **2.1449** 秒/输出帧(80 帧 171.59 s,7680×4320 输入),
///   远超锚点 0.5186。注意该样本含 33 Mpx 的 JPG 读+写、而 -j 1:1:1 只有 1 读 1 写线程,**不是纯推理成本**;
///   但无论如何,锚点表在 4320p 上是**偏乐观**的,>4320p 的外推更不可信(【待实测】,用一个 4320p 单帧目录即可分离 I/O)。
///   补帧的**每进程固定地板**实测 ≈ **1.26 s**(2 帧进 4 帧出:1.54 s 总时长 − 4 × 0.069)。
///
/// ==== 覆盖不到的组合怎么办 ====
...
public static class PipelineOrderPlan
{
    ...
    public static readonly UpscaleRate[] UpscaleRates =
    {
        ...
        new("realesrgan", "animevideov3", 3, 0.282,  "2026-09-15 实测 0.264~0.299(40 帧 1080p 目录批跑 ×6,取冷态样本);同模型 2x=0.252~0.269、4x=0.294~0.300 —— 三档几乎同价,是「成本 ∝ 源面积」的又一例证"),
        ...
        new("realesrgan", "x4plus", 4, 15.87,       "2026-09-15 实测 15.87【30 帧 1080p 目录批跑 475.96 s,GPU 72→87 ℃ 由冷态起跑 —— 已取代原先「仅 12 帧样本」的 15.145】;同机 4 个样本 10.19 / 13.21 / 15.87 / 19.05 s/帧,离散全部来自笔记本 GPU 热降频(throttle bit 0x20),与素材内容无关"),
