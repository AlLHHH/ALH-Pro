...
public class VideoPipelineTests
{
    ...
    // ---------- EstimateProcessSeconds:每批引擎启动开销(2026-09-13) ----------
    // 【为什么必须钉住】原公式只有"每帧成本",完全不知道有【批次】这回事:长素材被切成十几批、
    // 每批重启一次引擎进程,这笔固定开销一分钱没算 → "预计还剩 5 分钟"实际跑半小时。
    // 口径必须与真正执行时同源:批数一律走 RenderPolicy.PlanVideoBatches(用户口径:设备档位 + 源帧数 +
    // 补帧后总帧数 → 短素材单批 / 设备好长片 200~400 帧一批 / 设备差 50 帧一批)。
    ...
    public void Estimate_adds_batch_startup_overhead_when_free_ram_known()
    {
        ...
        Assert.Equal(1 * perBatch * 1.15, withRam - noRam, 6);   // 单批 = 1 次启动;收尾统一 ×1.15(与既有口径一致)
    }
