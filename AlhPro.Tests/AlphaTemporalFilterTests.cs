...
/// <summary>时序稳定(逐帧蒙版治抖动的核心)契约(2026-09-22)。三条判据,对应 AlphaTemporalFilter 的三个机制:
/// 静止更稳 / 真运动少拖影 / 切点必须重置。这三条是"能不能拿出去用"的分水岭:
/// 不稳 ⇒ 边缘逐帧呼吸、像有虫在爬;太稳 ⇒ 运动拖出鬼影;不重置 ⇒ 切镜后整个镜头挂着上一个镜头的形状。
/// 【2026-09-23】产生它的视频抠图页已下线,但这个类**有意保留**(用户:"留一点有用的"),
/// 所以这份契约继续守着它 —— 见 docs/2026-09-23-视频抠图下线-保留的经验与可复用件.md §二.1。
/// </summary>
