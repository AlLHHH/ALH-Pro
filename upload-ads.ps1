# ALH Pro 广告一键上传 (PowerShell 版, UTF-8 可靠)
# 用法: 双击"一键上传广告.bat"即可,它会调用本脚本。
...
Write-Step '== 检测 ad 目录改动 =='
$status = (git status --porcelain ad) -join "`n"
if ([string]::IsNullOrWhiteSpace($status)) {
    Write-Host ''
    Write-Host '当前 ad 目录【没有待上传的改动】。' -ForegroundColor Yellow
    Write-Host '请先去 D:\deep\alh-pro\ad 里修改要上传的广告文案或图片,再运行本脚本。'
...
$tmpMsg = Join-Path $env:TEMP 'alh_ad_msg.txt'
...
if ($pushOk) {
    ...
    Write-Host '  用户端最长 10 分钟看到新广告;刚打开软件的用户会立刻看到。' -ForegroundColor Green
    Write-Host '==========================================' -ForegroundColor Green
} else {
    Write-Host ''
