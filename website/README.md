...
1. **被刷流量 / 账单暴涨(最常见)** —— 别人把站上的图片/CSS 嵌到自己页面,或用脚本狂刷 CDN。
   防护:**CDN 层**开「防盗链(Referer 白名单)」+ 设「用量封顶/告警」**(后者最重要)。
   ⚠ **防盗链只做在 CDN 层,绝不要做在 OSS 桶层** —— 理由见下面那条真实事故。
...
### 二·补:桶层防盗链的真实事故(2026-09-22,务必看完再动防盗链)
**现象**:别人用手机访问站点,看到
`AccessDenied — You are denied by bucket referer policy`(HostId `alhpro.oss-cn-hangzhou.aliyuncs.com`,桶名 `alhpro`,错误码 `0003-00000503`)。

**实测(当天在这台机器上跑的)**:

| 访问方式 | 结果 |
|---|---|
| 直连源站 `alhpro.oss-cn-hangzhou.aliyuncs.com/index.html`,**不带 Referer** | 200 ✓ |
| 直连源站 + `Referer: https://alhpro.cn/` | 200 ✓(在桶的白名单里) |
| 直连源站 + **微信内置浏览器 Referer**(`https://mp.weixin.qq.com/`) | **403 桶防盗链拒绝** ✗ |
| 直连源站 + 其它任意站点 Referer | **403** ✗ |
| **走 `https://alhpro.cn/`(CDN)** + 微信 / QQ 内置浏览器 Referer | **200** ✓(CDN 回源**不透传**访问者的 Referer) |

**结论**:桶层防盗链**只会误伤真实场景**,挡不住真盗链 ——
被拒的一定是"**直连 OSS 域名**"的请求,而这类请求的典型来源正是**微信 / QQ / 抖音内置浏览器打开别人分享的 OSS 直链**
(内置浏览器会带上自己的 Referer,不在白名单里)✗;而正常走 `alhpro.cn` 的访问完全不受影响 ✓。

**正确做法(按推荐度)**

1. **桶的防盗链关掉**;要防盗链就开在 **CDN 层**(CDN 的 Referer 防盗链支持"允许空 Referer",且作用于真实访客);
2. 更彻底:**桶设为私有 + CDN 回源鉴权** —— 别人即使拿到 OSS 直链也读不到,而通过 `alhpro.cn` 一切照常;
3. 无论开在哪一层,**必须允许"空 Referer"**(直接输入网址、App 内置浏览器、部分爬虫都不带 Referer);
4. 对外分享链接时,**一律用 `https://alhpro.cn/...`,不要分享 OSS 直链**。

### 二·补2:**白名单式** Referer 防盗链对"公开官网"同样是坑(2026-09-22 当天实测,结论:别开)

在 CDN 上按"白名单只有 `*.alhpro.cn`"配好之后,实测**从站外点进来的真人全被 403**:

| 真人从哪点进来 | 结果 |
|---|---|
| 百度搜索结果 | **403** ✗ |
| Google / 必应 / 搜狗 / 360 | **403** ✗ |
| 微信 / QQ 里点开站点链接 | **403** ✗ |
| 微博 / 抖音里点开 | **403** ✗ |
| 直接输网址、搜索引擎爬虫(**不带 Referer**) | 200 ✓ |
| 站点自家页面(Referer 是 alhpro.cn) | 200 ✓ |

**根因**:搜索结果页、微信/QQ/抖音内置浏览器打开链接时都会带**自己的** Referer ⇒ 不在白名单里 ⇒ 403。
也就是说这条规则**主要拦的不是盗链的坏人,而是"从搜索引擎/社交软件点进来的真人"** ✗ ——
对刚做完 SEO、指望搜索与分享带量的公开官网,这是**致命误伤**(等于把入口流量全砍掉)。

**公开官网的正确做法**

1. **入口页面(HTML)永远不要做 Referer 白名单** —— 它是"从哪都能点进来"的东西;
2. 真要防图片/CSS 被盗嵌,**只对静态资源目录做**,并且用 CDN 规则里的「**规则条件**」限定路径
   (例如只对 `assets/` 生效)—— 页面自己加载资源时 Referer 是自家域名 ✓ 正常,别的网站嵌你的图 → 403 ✓;
3. **防刷的核心不是防盗链,而是「用量封顶 + 告警」** —— 防盗链挡不住脚本(可伪造/可空),封顶能兜住一切;
4. 想彻底防"直连盗用":**私有桶 + CDN 回源鉴权**(不影响访客,也不误伤任何真人)。

> 两条事故连起来看:**桶层防盗链**误伤了"分享 OSS 直链"的场景,**CDN 层白名单防盗链**误伤了"从搜索/社交点进来"的场景 ——
> 共同教训是:**防盗链只在"静态资源"这一层、并且明确知道要放行谁的时候才用;公开站点的入口页面一律不用。**

### 三、每个季度顺手自查一遍(5 分钟)

- [ ] OSS 桶「读写权限」= 公共读 / 私有(**不是**公共读写)
- [ ] OSS 桶的「防盗链」**是关着的**(防盗链开在 CDN 层 —— 见上面那条事故)
- [ ] CDN「防盗链」:**入口页面(HTML)不要做 Referer 白名单**;若要防图片盗嵌,只对 `assets/` 这类静态资源目录开(用「规则条件」限定路径),并**允许空 Referer**
