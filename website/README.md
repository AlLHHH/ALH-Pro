...
| **减少动态效果** | 唯一的位移动效是"滚动淡入 + 上移 8px",已挂在 `prefers-reduced-motion: reduce` 下关闭。**加新动效时不要漏这一条**(见 `style.css` §18) |
| **设计克制** | 底色只用中性灰阶,只有一个强调色;边框统一 1px;阴影只做"提升"不做"发光"。不引入逐词浮现、鼠标跟随光晕、漂浮光斑、滚动视差一类效果(见 `style.css` 开头的八条硬规则) |
| **体积预算** | `style.css` + `site.js` 合计控制在 **100 KB** 以内(当前约 43 KB)。想加效果请手写 CSS/JS,不要引入外部库 |
...
├── assets/
│   ├── css/style.css   全部样式。开头是"八条硬规则"与 §1 设计令牌(配色 / 圆角 / 阴影 / 缓动),
│   │                   改深浅两套主题只改 §1;§18 是 prefers-reduced-motion 的统一关闭规则
│   ├── js/site.js      全部交互,按 §0~§6 分模块的小函数:
│   │                   手机端导航折叠 / 页内章节条与当前章节高亮 / 滚动进入动画 /
...
> `data-reveal` = 该区块滚动进入时淡入上移 8px;`data-reveal-group` = 放在容器上,
> 让子元素按 40ms 依次错开(最多 4 级)。
