...
| **零外部依赖** | 不引用任何境外 CDN、在线字体、外部 JS 库、统计脚本。字体全部走系统字体栈(Windows 上是微软雅黑) |
| **可离线打开** | 直接双击 `index.html` 就能看;所有资源都是相对路径 |
| **移动端自适应** | 手机 / 平板 / 桌面三档布局;导航在窄屏折叠(无 JS 时自动展开为可换行的列表) |
| **深色模式** | 跟随系统 `prefers-color-scheme` 自动切换,无需手动开关 |
...
├── assets/
│   ├── css/style.css   全部样式(改颜色只需改文件开头的 CSS 变量)
│   ├── js/site.js      仅三件事:手机端导航折叠、复制按钮、当前页高亮兜底
│   └── img/logo.png    站点图标 / 页眉标志(取自软件图标 ALHPro_icon.png)
└── README.md           本文件
```
