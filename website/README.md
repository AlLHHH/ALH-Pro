...
| **滚动性能铁律** | ① **禁止 `backdrop-filter`**(大面积用它会显著拖慢滚动);② **禁止在滚动相关代码里调用任何 scroll API**(`scrollTo`/`scrollIntoView`/`scrollTop=`)—— 这条是"滚动时被拽着走"的根因,详见 `site.js` 顶部说明;③ 滚动路径上只动 `transform`/`opacity`,滚动联动优先用 CSS 滚动驱动动画 |
| **体积预算** | `style.css` + `site.js` 合计控制在 **100 KB** 以内(当前约 63 KB)。想加效果请手写 CSS/JS,不要引入外部库 |
...
│   ├── js/site.js      全部交互,按 §0~§12 分模块的小函数:
